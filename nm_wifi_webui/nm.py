...
from txdbus import (
    error as txdbus_error,
    client as txdbus_client,
    objects as txdbus_objects,
    interface as txdbus_iface,
...
def ntop(n, af, none_for_0=False, _afid={4: socket.AF_INET, 6: socket.AF_INET6}):
    assert af in [4, 6], af
...
class NMInterface(service.MultiService):
    ...
    def _startService(self):
        ...
        if not self.wifi_iface:
            self.log.debug(
                "Unable to detect valid wifi interface, waiting for one to become available"
            )
    ...
    wifi_conn_res = set(
        [
            ...
            "live_init",
            "live_connect",
            "live_config",  # connect = prepare-auth, config = ip_* and later
            "done",
            "fail_auth",
            "fail_addr",
            "fail_link",
    ...
    wifi_conn_res_text = dict(
        idle_disconnected="Idle",
        idle_unavailable="Unavailable",
        live_init="Initiating connection",
        live_connect="Authenticating",
        live_config="Configuring network parameters",
        done="Idle",
        fail_auth="Authentication failed",
        fail_addr="Network configuration failed",
        fail_link="Network became unreachable",
        fail="Association failed",
    )
    ...
    def iface_added(self, dev_path):
        ...
        def skip(reason, level="warn"):
            getattr(self.log, level)(
                "Skipping wifi interface (reason: %s): %s", reason, iface
            )
            defer.returnValue(None)
    ...
    def iface_changed_apply(self, dev, new_state, old_state=None, reason=None):
        new_state, old_state = list(
            map(ft.partial(dev.interpret_prop_value, "State"), [new_state, old_state])
        )
        ...
        log_status = lambda msg, level="debug": getattr(self.log, level)(
            "%s (dev: %s/%s): %s -> %s, %s",
            msg,
            iface_type,
            iface,
        ...
        if dev_is_wifi:
            ...
            if new_state == "failed":
                ...
                if old_state == "need_auth" and reason == "no_secrets":
                    self.wifi_update_ap(
                        uid=self.wifi_conn_res_ap.uid, pass_state="error"
                    )
                ...
                if self.wifi_conn_res_code == "live_connect":
                    ...
                    self.log.warn(
                        ...
                        new_state,
                        reason,
                        self.wifi_conn_res_fail_reason,
                    )

            elif new_state in self.wifi_conn_res_dict:  # "in-progress"
                if self.wifi_conn_res_dict[new_state] != self.wifi_conn_res_dict.get(
                    old_state
                ):
                    if old_state == "config" and new_state == "ip_config":
                        self.wifi_update_ap(
                            uid=self.wifi_conn_res_ap.uid, pass_state="success"
                        )
                ...
                self.log.warn(
                    "Unhandled connection state" " transition: %s -> %s, %s",
                    old_state,
                    new_state,
                    reason,
                )
    ...
    def wifi_init(self):
        ...
        for sig_ref_set in self.wifi_signals_ap.values():
    ...
    def wifi_aps_list(self):  # XXX: maybe just don't put deferreds in there?
        aps = list()
        for k in list(self.wifi_aps.keys()):
    ...
    def wifi_added(self, iface, dev, sec_id):
        ...
        self.wifi_signals.extend(
            [
                (
                    yield dev.signal(
                        "AccessPointAdded",
                        self.wifi_update_ap_added,
                        iface="Device.Wireless",
                    )
                ),
                (
                    yield dev.signal(
                        "AccessPointRemoved",
                        self.wifi_update_ap_removed,
                        iface="Device.Wireless",
                    )
                ),
                (
        ...
        self.log.debug(
            "Using new wifi interface:" " %s (capabilities: %s)",
            self.wifi_iface,
            ", ".join(self.wifi_sec),
        )
    ...
    def wifi_removed(self, reason="unspecified"):
        ...
        self.log.debug(
            "Resetting wifi interface (%s," " reason: %s), scheduling detection task",
            self.wifi_iface,
            reason,
        )
        yield self.wifi_conn_enabled_set(False)
    ...
    def wifi_status_update(self, code, status, action=None, ap=None, config=None):
        ...
        assert not code or not (
            (code.startswith("fail_") or code.startswith("live_")) and not ap
        ), [code, ap]
        if action is None:
            action = self.wifi_conn_res_text[code]
        self.log.debug(
            "Status update: %s, %s" " (code: %s, ap: %s, config: %s)",
            status,
            action,
            code,
            ap,
    ...
    def wifi_status_update_fail(
        self, code, status="Disconnected", action=None, ap=None
    ):
        if ap is None:
    ...
    def wifi_status_update_ap(self):
        ...
        if conn_path != "/":
            ...
            ap_path = yield self.dbus.ref("Connection.Active", conn_path).get(
                "SpecificObject"
            )
    ...
    def wifi_connect(self, ap_info):
        ...
        def stop_if_ap_is_gone():
            if self.wifi_dev and ap.uid in self.wifi_aps:
                return
            defer.returnValue(
                self.wifi_status_update("fail_link", "Disconnected", ap=ap)
            )
        ...
        try:
            ...
            self.log.debug(
                "Reusing existing connection Settings (path: %s): %s",
                conn_path,
                settings,
            )
        ...
        if ap.sec != "none":
            ...
            if ap.sec.startswith("wpa-"):
                if len(ap_info["p"]) < 8:
                    defer.returnValue(
                        self.wifi_status_update_fail(
                            "auth",
                            action="WPA passphrase/key must be 8-32 characters long",
                            ap=ap,
                        )
                    )
                if ap.mode == "adhoc":
                ...
                pw_type = (
                    1 if pw_len in [5, 13] or (pw_hex and pw_len in [10, 26]) else 2
                )
                settings["802-11-wireless-security"] = {
                    "key-mgmt": "none",
                    "auth-alg": "shared",
                    "wep-key-flags": 1,
                    "wep-key-type": pw_type,
                }
        else:
        ...
        self.log.debug(
            "Activating connection with" " settings (update: %s): %s",
            bool(conn_obj),
            settings,
        )
        if conn_obj:
            yield conn_obj.Update(settings)
            active_path = yield self.dbus.nm.ActivateConnection(
                conn_path, self.wifi_dev_path, ap.dbus_path
            )
        else:
            conn_path, active_path = yield self.dbus.nm.AddAndActivateConnection(
                settings, self.wifi_dev_path, ap.dbus_path
            )
    ...
    @defer.inlineCallbacks
    def wifi_update_ap_changed(
        self,
        ap,
        props,
        as_dict=False,
        _as_is=dict(
            Strength="strength", Mode="mode", HwAddress="hwaddr", MaxBitrate="bitrate"
        ),
    ):
        ap_update = dict()
        for k, v in props.items():
...
def dbus_variant_sanitize(data, bytearrays=True, empty_lists=True):
    ...
    if isinstance(data, dict):
        data = dict(
            list(
                zip(
                    dbus_variant_sanitize(list(data.keys()), bytearrays=False),
                    dbus_variant_sanitize(list(data.values()), **kws),
                )
            )
        )
        if empty_lists:
            for k, v in list(data.items()):
                ...
                if empty_lists is True:
                    ...
                    data[k] = type(
                        "list_sig", (list,), dict(dbusSignature=empty_lists)
                    )()
    elif isinstance(data, list):
        data = list(map(ft.partial(dbus_variant_sanitize, **kws), data))
        if bytearrays and data and all((isinstance(v, int) and v <= 255) for v in data):
            data = list(map(txdbus_marshal.Byte, data))
    return data
...
class DBusRef(object):
    ...
    prop_wrappers = {
        ...
        "AccessPoint": dict(Mode={0: "unknown", 1: "adhoc", 2: "infrastructure"}),
    }
    ...
    def interpret_prop_value(self, k, v, strict=False):
        ...
        try:
            ...
            if strict or v is not None:
                self.log.warn(
                    "Unrecognized enum value for %s/%s: %s", self.iface_short, k, v
                )
            v = v_dict.get(
                None, v_dict.get(0)
...
class DBusProxy(object):
    ...
    def _dbus_connect(self):
        ...
        try:
            ...
            if not nm_alive:
                raise NMConnectionError(
                    "Failed to connect to NetworkManager dbus interface"
                )
    ...
    def _dbus_exports(self):
        ...
        reactor.callLater(
            0, lambda: self.ref("AgentManager").Register(nm_secret_agent_guid)
        )

    @defer.inlineCallbacks
    def _dbus_test(self):
        try:
            nm_obj = yield self._ref_resolve(
                self._conn, self.nm
            )  # doesn't grab _conn_lock
            perms = yield self.call(
                nm_obj, "GetPermissions", _interface=dbus_iface_nm_ns
            )
            if set(perms.values()).difference(["yes"]):
            ...
            self.log.debug(
                "Failed to query NetworkManager dbus interface: (%s) %s", err_type, err
            )
        else:
    ...
    def _dbus_disconnect_handler(self, nm_obj=None, reason=None, watchdog=False):
        if nm_obj is None and not watchdog:
            ...
            self._dbus_disconnect_task = task.LoopingCall(
                self._dbus_disconnect_handler, watchdog=True
            )
    ...
    def _process_obj_iface_specs(self, obj_or_ref, kws, defaults=None):
        ...
        assert isinstance(obj_or_ref, txdbus_objects.RemoteDBusObject), [
            type(obj_or_ref),
            obj_or_ref,
        ]
    ...
    def call(self, obj_or_ref, func, *args, **kws):
        obj = yield self._process_obj_iface_specs(
            obj_or_ref, kws, self.dbus_call_defaults
        )
        self.log.noise(
            "Call: %s %s %s %s %s",
            obj.objectPath,
            list(map(op.attrgetter("name"), obj.interfaces)),
            func,
            args,
            kws,
        )
    ...
    def _wrapper(self, *args, **kws):
        ...
        try:
            ...
            if not isinstance(err, DBusSecretAgentError):
                self.log.exception(
                    "Call[%s] unhandled error: (%s) %s", call_id, err_type, err
                )
            else:
...
nm_secret_agent_guid = (
    "nah.naah.naaaah.SecretAgent"
)  # XXX: some proper product-related guid?
...
class DBusSecretAgent(txdbus_objects.DBusObject):
    ...
    def dbus_GetSecrets(self, connection, connection_path, setting_name, hints, flags):
        ...
        if connection["802-11-wireless-security"].get("key-mgmt") != "none":  # wpa
            ...
            if flags & 0x02 and pw == pw_prev:  # prev auth with stored key has failed
                self.secrets.update(
                    ap_uuid, state="error"
                )  # redundant with no_secrets nm-state-signal
                pw = None
            else:
                sec = {"psk": pw}
        else:  # wep
    ...
    @_dbus_log_calls
    def dbus_SaveSecrets(self, connection, connection_path):
        pass

    @_dbus_log_calls
    def dbus_CancelGetSecrets(self, connection_path, setting_name):
        pass

    @_dbus_log_calls
    def dbus_DeleteSecrets(self, connection, connection_path):
        pass
