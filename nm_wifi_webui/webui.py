...
class WebUI(resource.Resource):
    ...
    def dispatch(self, ev):
        ...
        assert (ev["q"] not in ["result", "status"] and ev.get("ap")) or (
            ev["q"] != "result" or ev["id"]
        ), ev
        assert ev["q"] != "status" or (ev.get("status") and ev.get("action")), ev
        self.events.send(ev)

    def dispatch_status(
        self, status=None, action=None, code=None, ap_uid=None, config=None
    ):
    ...
    def handle_command(self, ev):
        assert (
            ev["q"] in ["online", "connect", "disconnect", "scan", "sync", "auto"]
            and ev["id"]
        ), ev
        dispatch = lambda r, **kw: self.dispatch(
            dict(q="result", id=ev["id"], r=r, **kw)
        )
    ...
    def handle_status_update(self, status, action, code, ap, config):
        self.dispatch_status(
            status=status, action=action, code=code, ap_uid=ap and ap.uid, config=config
        )
    ...
    def ap_info(self, ap):
        ...
        ap["title"] = "\n".join(
            [
                ...
                "Bitrate: {rate:.1f} Mbps",
                "BSSID: {0[hwaddr]}",
            ]
        ).format(
            ap,
            rate=ap["bitrate"] / 1000.0,
            sec="Open Access Point"
            if not ap["private"]
            else "Security: {}".format(ap["sec"]),
        )
    ...
    def get_ap_data(self):
        return list(
            map(
                self.ap_info,
                sorted(self.nm.wifi_aps.values(), key=op.attrgetter("uid")),
            )
        )
    ...
    def get_config_params(self, config):
        ...
        data = [
            "Address{}: {}".format(
                "es" if len(addrs) > 1 else "", ", ".join(sorted(addrs))
            )
        ]
    ...
    def render_GET(self, request):
        ...
        nm_connection = not (nm_code.startswith("idle_") or nm_code.startswith("fail_"))
        ...
        env = dict(
            ...
            events_url=utils.build_url(sock.host, sock.port, path=self.url_events),
        )
    ...
    def render_nm_connection_result(self, request, ap):
        try:
            res = yield utils.timeout(self.post_nm_wait_max, self.nm.wifi_connect(ap))
        except:
        ...
        if res is None:
            ...
            request.write(
                self.templates.get_template("static_nm_redirect.html")
                .render(url_base=dst_url)
                .encode("utf-8")
            )
        else:
